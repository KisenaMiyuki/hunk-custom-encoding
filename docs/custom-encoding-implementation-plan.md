...
- **宿主环境要求（2026-09-12 定案）**：扩展需要提供 extension API **v25** 的宿主（对应 hunkdiff@0.22.0）。曾怀疑 npm `hunkdiff-windows-x64@0.22.0` 预编译二进制是陈旧构建（v16/0.21.1）——**该结论错误，已撤回**：registry 的 0.22.0 tarball 自发布起就是好的（内嵌 API = 25，`--version` = 0.22.0）。真实根因是 **bun 全局更新的部分替换**：从 hunkdiff@0.21.1 升级时，`hunkdiff-windows-x64` 目录里 package.json/LICENSE 被换成 0.22.0 内容（mtime Sep 11 13:12），但硬链接的 `bin/hunk.exe` 仍是 0.21.1 的本体（mtime Sep 10 15:15、hardlink 数 2、sha256 `fbe0d8d7…` 与 0.21.1 缓存条目一致），造成"包名 0.22.0、二进制 0.21.1"的混合状态；peer（@opentui 0.5.4）同理未随更新刷新。修复：`bun remove -g hunkdiff && rm -rf node_modules/hunkdiff-windows-x64* && bun add -g hunkdiff`（彻底重装），并把 peer 升到 `@opentui/core@0.5.11`（0.5.4 的 DLL 缺 `createEmbeddedTerminal`，v25 JS 宿主需要）。重装后标准预编译二进制路径已端到端验证：扩展加载、GBK 正常转码。候选后续：可向 oven-sh/bun 报告"全局更新未替换硬链接二进制/未刷新 optional+peer 依赖"的行为（证据：缓存双条目 + mtime 时间线）。

### 里程碑现状（M0–M3 已落地，2026-09-12）
